...
for row, benchmark in enumerate(benchmarks):
    ...
    for i, (ax, fraction_sampled_points) in enumerate(zip(axs[row, :], all_fraction_sampled_points)):
        ...
        sns.lineplot(ax=ax, data=plot_df, x='Number of source models', y=f'MDE', hue='Technique', palette=palette, marker='o', markersize=10, alpha=0.8, linewidth=4, zorder=10, clip_on=True)
        for technique in techniques:
            technique_df = plot_df.loc[(plot_df['Technique'] == technique)]
            data = sorted(zip(technique_df['Number of source models'].tolist(), technique_df['MDE'].tolist(), technique_df['MDE lower CI'].tolist(),  technique_df['MDE upper CI'].tolist()))
