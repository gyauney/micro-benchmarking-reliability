...
save_dir = f'graphs-combine-subtasks_{mde_correctness}-mdad-correctness'
...
def multiple_panel_figure_x_axis_fraction_sampled(benchmark, df_mde, full_estimation_df, df_correctness):   
    ...
    savefig(f'{save_dir}/{benchmark}_combine-subtasks_full-results-panel.pdf', bbox_inches='tight')
    plt.close()
...
def print_full_results_table(benchmark, df_mde, full_estimation_df, df_correctness):
    split = 'unseen'
    techniques = ['Random (uniform)', 'Random (subtask stratified, equal)', 'Random (subtask stratified, proportional)', 'Anchor points (weighted)', 'tinyBenchmarks',]
    for num_source_models in [10, 50, 100, 150, 200, 250, 300]:
        for fraction_sampled_points in all_fraction_sampled_points:
            plot_df = full_estimation_df.loc[(full_estimation_df['Number of source models'] == num_source_models) \
                                             & (full_estimation_df['Technique'] == 'Random') \
                                             & (full_estimation_df['Fraction of sampled points'] == fraction_sampled_points)]
            
            full_size = benchmark_to_full_seen_size[benchmark]

            row = f'{benchmark_to_display_name[benchmark]};{num_source_models};{fraction_sampled_points/full_size*100:.1f}%;{fraction_sampled_points};'
            # estimation error
            # kendall tau
            # average correctness
            # minimum accuracy difference
            for technique in techniques:
                plot_df = full_estimation_df.loc[(full_estimation_df['Number of source models'] == num_source_models) \
                                                & (full_estimation_df['Technique'] == technique) \
                                                & (full_estimation_df['Fraction of sampled points'] == fraction_sampled_points)]
                estimation_error = plot_df[f'Mean estimation error against {split} accuracies'].mean()
                row += f'{estimation_error:.4f};'
            for technique in techniques:
                plot_df = full_estimation_df.loc[(full_estimation_df['Number of source models'] == num_source_models) \
                                                & (full_estimation_df['Technique'] == technique) \
                                                & (full_estimation_df['Fraction of sampled points'] == fraction_sampled_points)]
                kendall_tau = plot_df[f'Kendall tau correlation against {split} accuracies'].mean()
                row += f'{kendall_tau:.4f};'
            for technique in techniques:
                plot_df = df_correctness.loc[(df_correctness['Number of source models'] == num_source_models) \
                                                & (df_correctness['Technique'] == technique) \
                                                & (df_correctness['Fraction of sampled points'] == fraction_sampled_points)]
                correctness = plot_df[f'Average correctness {split} (mean)'].iloc[0]
                row += f'{correctness:.4f};'
            for technique in techniques:
                plot_df = df_mde.loc[(df_mde['Number of source models'] == num_source_models) \
                                    & (df_mde['Technique'] == technique) \
                                    & (df_mde['Split'] == split) \
                                    & (df_mde['Fraction of sampled points'] == fraction_sampled_points)]
                mde = plot_df[f'MDE'].iloc[0]
                row += f'{mde:.04f};'
            print(row)


benchmarks = ['mmlu-pro', 'mmlu', 'mmlu-pro', 'bbh', 'gpqa']
...
for benchmark in benchmarks:
    ...
    if not redo and os.path.exists(cached_mde_results_fn) \
      ...
      and os.path.exists(cached_mde_results_fn):
        ...
        for num_source_models in all_num_source_models:
            ...
            fn = f'{results_dir}/results_{benchmark}_{num_source_models}-source-models_50-runs.json'
            with open(fn, 'r') as f:
                results = json.load(f)
            tidy_results = []
            ...
            for fraction_sampled_points in tqdm(all_fraction_sampled_points):
                ...
                for technique in techniques:
                    ...
                    for split in ['Seen', 'Unseen']:
                        mde, mde_lower, mde_upper = get_mde_with_error_bars(df, split=split, mdad_threshold=mde_correctness)
    ...
    # multiple_panel_figure_x_axis_num_source_models(benchmark, df_mde, full_estimation_df, df_correctness)
