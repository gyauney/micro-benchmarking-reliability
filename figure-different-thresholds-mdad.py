...
for ax_col, benchmark in enumerate(benchmarks):
    ...
    for ax_row, mdad_threshold in enumerate(['0.7', '0.8', '0.9']):
        if mdad_threshold in ['0.7', '0.9']:
            cached_mde_results_fn = f'graphs-combine-subtasks_{mdad_threshold}-mdad-correctness/{benchmark}-mde-results.csv'
        else:
            cached_mde_results_fn = f'graphs-combine-subtasks/{benchmark}-mde-results.csv'

        if not os.path.exists(cached_mde_results_fn):
        ...
        plot_df = df_mde.loc[(df_mde['Split'] == split) \
